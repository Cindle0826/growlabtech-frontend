...
const SigninForm = () => {
    ...
    const googleAuthUrl = process.env.NEXT_PUBLIC_GOOGLE_OAUTH2_URL;
    ...
    const scope = process.env.NEXT_PUBLIC_GOOGLE_OAUTH2_SCOPES;
    const responseType = "code";
    const state = window.location.href; // 保存原始請求 URI
    const accessType = "offline"; // 添加此參數以獲取 refresh token
    const prompt = "consent";     // 添加此參數以確保每次都獲得新的 refresh token

    // Build Auth URL
    const authUrl = `${googleAuthUrl}?client_id=${clientId}&redirect_uri=${encodeURIComponent(
        redirectUri
    )}&response_type=${responseType}&scope=${encodeURIComponent(scope)}&state=${encodeURIComponent(state)}&access_type=${accessType}&prompt=${prompt}`;
